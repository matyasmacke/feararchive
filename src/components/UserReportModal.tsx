...
import { AlertTriangle, Flag, Loader2, UserRound, X } from 'lucide-react';
import { USER_REPORT_REASON_LABELS } from '../types';
import type { UserReportReason } from '../types';

export function UserReportModal({ username, onClose, onSubmit }: {
  username: string;
  onClose: () => void;
    ...
    >
      <form onSubmit={handleSubmit} className="w-full max-w-lg overflow-hidden rounded-2xl border border-red-900/40 bg-gray-950 shadow-2xl shadow-red-950/30">
        <div className="flex items-start justify-between gap-4 border-b border-red-900/30 px-5 py-4 sm:px-6">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <h2 id="user-report-title" className="text-lg font-bold text-white">Report User</h2>
        ...
        <div className="space-y-5 px-5 py-5 sm:px-6">
          ...
          <div>
            <label htmlFor="user-report-reason" className="mb-2 block text-sm font-medium text-gray-300">Reason</label>
            <select
              id="user-report-reason"
              value={reason}
              onChange={event => setReason(event.target.value as UserReportReason)}
            ...
            >
              {(Object.entries(USER_REPORT_REASON_LABELS) as [UserReportReason, string][]).map(([value, label]) => (
          ...
          <div>
            <div className="mb-2 flex items-center justify-between gap-3">
              <label htmlFor="user-report-details" className="text-sm font-medium text-gray-300">Additional details <span className="font-normal text-gray-600">(optional)</span></label>
            ...
            <textarea
              ...
              maxLength={1000}
              rows={4}
              placeholder="Briefly explain what the moderation team should review..."
